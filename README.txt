...
- I have initially missed the requirement to use for the ccxt library so I made a new KuCoin account and made the requests go through their official API. I'm adding the 3 line ccxt snippet in the same request handler as a comparison
...
- We assume the database is present on localhost with credentials. If not, it's an easy extension to create and populate the database using sqlalchemy